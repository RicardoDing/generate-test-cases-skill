...
本文档定义功能测试的5种核心设计方法（EP/BVA/PV/ST/EG），用于指导 AI 生成专业、全面的测试用例。
...
| 涉及业务流程/多步骤操作 | 场景法 |
| 历史缺陷多/高风险模块 | 错误推测法 |
| 多字段组合约束 | 等价类划分 + 错误推测（补充关键组合） |

> **注**：本文档覆盖 5 种核心方法（EP/BVA/PV/ST/EG）。对于多因素组合场景，如需更系统的覆盖，可考虑判定表法或正交实验法，但这些不在核心方法范围内。
...
| 错误类型 | 测试场景 |
|---------|---------|
| 重复提交 | 快速双击提交按钮，应防止重复创建记录（如重复下单、重复支付） |
| 并发操作 | 多窗口/多Tab同时操作同一数据。示例：两个用户同时编辑同一条记录，后提交者应有冲突提示或乐观锁拦截 |
| 重复请求 | 网络延迟时用户多次点击，服务端应具备幂等性，不产生重复数据 |
...
| 优先级 | 覆盖内容 |
|--------|---------|
| **P0** | 基本流用例 + 基本流涉及的有效等价类（即主成功路径所需的合法输入） |
| **P1** | 备选流用例 + 边界值 + 主要无效等价类（如必填校验、格式校验） |
...
每次生成后统计设计方法分布，写入 `generation-history.json`（结构定义见 [MEMORY-SCHEMA.md](MEMORY-SCHEMA.md)）。若发现方法占比异常，Phase 2.8 预览时提示调整：

| 方法 | 正常占比参考 | 异常阈值 | 触发动作 |
|------|------------|---------|---------|
| EP 等价类 | 20-35% | < 15% 或 > 40% | 提示检查是否遗漏输入域划分 |
| BVA 边界值 | 10-20% | < 5% 或 > 25% | 提示检查是否遗漏边界场景 |
| PV 参数验证 | 15-30% | < 10% 或 > 35% | 提示检查参数覆盖完整性 |
| ST 场景法 | 10-25% | < 5% 或 > 30% | 提示检查业务流程覆盖 |
| EG 错误推测 | 5-15% | > 30% | 提示 EG 占比过高，可能存在冗余 |

> **注**：以上阈值为经验参考值，非硬性约束。补充方法（RV/BS/RL/DB/REDIS）的占比取决于接口特征，不设固定阈值。

其他自学习规则：
- 某模块历史上漏测特定方法 → 下次自动补充
- 用户反馈删减某类用例 → 记录偏好，下次降低该方法权重
