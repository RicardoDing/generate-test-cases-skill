...
|---------|-----------|------|
| 基本流（场景法） | P0 | 核心路径必须通过 |
| 有效等价类（核心业务输入） | P0 | 核心业务流程的正常输入（如登录的正确账号密码） |
| 有效等价类（非核心业务输入） | P1 | 非核心功能的正常输入（如个人设置的合法值） |
| 边界值（上下边界） | P1 | 边界易出错 |
| 备选流（场景法） | P1 | 替代路径需验证 |
| 无效等价类（影响业务流程） | P1 | 可能导致业务中断的异常输入（如必填字段为空） |
| 无效等价类（不影响业务流程） | P2 | 不影响主流程的异常输入（如备注字段超长） |
| 异常流（场景法） | P2 | 错误处理验证 |
| 错误推测（常见） | P2 | 经验性补充 |
| 错误推测（边缘） | P3 | 极端场景 |
...
| 风险等级 | 用例优先级分布 | 建议覆盖深度 |
|---------|---------------|-------------|
| 极高风险 | P0 40% + P1 40% + P2 20% | 建议 ≥8 用例/需求 |
| 高风险 | P0 30% + P1 40% + P2 30% | 建议 ≥5 用例/需求 |
| 中风险 | P0 20% + P1 40% + P2 40% | 建议 ≥3 用例/需求 |
| 低风险 | P0 10% + P1 30% + P2 60% | 建议 ≥2 用例/需求 |

> **注意**：覆盖深度为参考值，应根据需求的验收条件数量和复杂度灵活调整。若需求验收条件较少，不必为凑数而生成低价值用例。
...
- 每个核心模块的基本流
- 登录、核心业务主流程
- 参考范围：20-30 条（按项目规模动态调整，不作为硬上限）

**核心回归选择**：
- 所有冒烟用例
- 主要功能的正向+负向用例
- 上次迭代修复的缺陷回归
- 参考范围：100-200 条（按项目规模动态调整，不作为硬上限；大型系统可适当放宽）
